...
type LogMessage = string | LogMeta;
...
export class PinoLoggerService implements LoggerService {
	 ...
	 */
	private buildContext(meta?: LogMeta): LogMeta {
		return {
			...(meta ?? {}),
	...
	error(message: LogMessage, trace?: string, context?: string): void {
		const logContext = context || this.context;
		if (typeof message === "string") {
			this.logger.error(this.buildContext({ context: logContext, trace }), message);
		} else {
			this.logger.error(this.buildContext({ ...message, context: logContext, trace }));
		}
	}
	 ...
	 */
	child(bindings: LogMeta): PinoLoggerService {
