...
import type { ISqsClient } from "./sqs-client.interface";
 ...
 */
@Injectable()
export class SqsClientMockService implements ISqsClient<unknown> {
	private readonly logger = new Logger(SqsClientMockService.name);
	private readonly messages: Map<string, unknown[]> = new Map();
	...
	async sendBatch(queueName: string, messages: unknown[]): Promise<string[]> {
	 ...
	 */
	getMessages(queueName: string): unknown[] {
